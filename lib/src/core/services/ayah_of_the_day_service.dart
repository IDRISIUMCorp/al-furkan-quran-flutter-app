...
class AyahOfTheDayService {
  ...
  static Future<void> setupBackgroundUpdates() async {
    await Workmanager().initialize(ayahWidgetCallbackDispatcher, isInDebugMode: false);
  ...
  static Future<void> updateWidget({bool forceRefresh = false}) async {
    ...
    final widgetFontSize = box.get("widget_font_size", defaultValue: 28.0) as double;
    final widgetTheme = box.get("widget_theme", defaultValue: "glass_dark") as String;
    ...
    final isGradientBg = box.get("widget_is_gradient_bg", defaultValue: false) as bool;
    ...
    final customBgColor = customBgColorInt != null ? Color(customBgColorInt) : null;
    final customBgColor2 = customBgColorInt2 != null ? Color(customBgColorInt2) : null;
    final customTextColor = customTextColorInt != null ? Color(customTextColorInt) : null;
    final customSurahColor = customSurahColorInt != null ? Color(customSurahColorInt) : null;
    
    // Read the user's theme primary color; fallback to default teal
    final int primaryColorInt = box.get("primaryColor", defaultValue: 0xFF33B18E) as int;
    ...
    // Get the Arabic text and Surah Name
    String ayahText = getVerse(surah!, verse!, verseEndSymbol: false);
    
    // Remove unwanted HTML tags or formatting if standard getVerse has them
    ayahText = ayahText.replaceAll(RegExp(r'<[^>]+>'), '');
    
    // Format text nicely (like replacing the first space with non-breaking space if it's short)
